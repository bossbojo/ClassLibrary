...
namespace ClassLibrary.Pagination
{
    public class PaginationAndFilter
    {
        ...
        private PaginationParamsRequest GetParamsRequest()
        {
            ...
            int ARF_PAGE = string.IsNullOrEmpty(Request["ARF_PAGE"]) ? 1 : int.Parse(Request["ARF_PAGE"]);
            int ARF_LIMIT = string.IsNullOrEmpty(Request["ARF_LIMIT"]) ? 15 : int.Parse(Request["ARF_LIMIT"]);
            string ARF_FILTER = string.IsNullOrEmpty(Request["ARF_FILTER"]) ? "" : Request["ARF_FILTER"];
